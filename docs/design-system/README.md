# Design System — ACT Strive

| File | Status |
|------|--------|
| [`DESIGN_SYSTEM_BRIEF.md`](./DESIGN_SYSTEM_BRIEF.md) | v1.0 — **FREEZE** (ADR-001) |
| [`TOKENS.md`](./TOKENS.md) | Brand + light/dark |
| [`COMPONENTS.md`](./COMPONENTS.md) | **ErpTable** spec wajib |
| [`assets/act-strive-128.svg`](./assets/act-strive-128.svg) | Logo legacy |
| [`wireframes/R1-list-purchase-order.md`](./wireframes/R1-list-purchase-order.md) | List pattern |
| [`wireframes/R2-detail-purchase-order.md`](./wireframes/R2-detail-purchase-order.md) | Detail + approval |

Tracker: [`../DISCUSSION_AGENDA.md`](../DISCUSSION_AGENDA.md)
